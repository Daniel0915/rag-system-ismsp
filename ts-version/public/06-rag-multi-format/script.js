const API = "/api/06-rag-multi-format";
...
uploadBtn.addEventListener("click", async () => {
  ...
  uploadResult.textContent = "처리 중... (이미지는 Gemini Vision 분석이 포함되어 다소 걸릴 수 있어요)";
  try {
...
async function sendChat() {
  ...
  try {
    ...
    for (const s of sources) {
      ...
      div.textContent = `[${s.fileType}] ${s.sourceFile}${s.page ? ` — 페이지 ${s.page}` : ""}`;
      if (s.fileType === "pdf") {
